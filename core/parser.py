...
class Parser:
    ...
    def parse(self, host, port, vhost, args, global_variables, vhosts, output_queue, parser_queue, global_state):
        state = {}
        output = ''
        try:
            ...
            try:
                response = requests.get('http://%s:%s/' % (host, port), verify=False, timeout=self.settings['TIMEOUT'],
                                        proxies=self.settings['PROXIES'], allow_redirects=False)
            ...
            for priority in range(0, MAX_PRIORITIES):
                for module_name, module in self.modules['peri-modules'].items():
                    if module['priority'] == priority:
                        ...
                        try:
                            module_output, module_state = module['obj'].run_peri_module(state_copy, self.settings, self.modules, self.coloring)
